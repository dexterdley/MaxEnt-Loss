...
def solve_var_lagrange(x, mu, var, lam=0, max_iter = 20, tol = 1e-15):
    #Implements the Newton Raphson method:
    i = 0
    old_lam = lam

    def fx_var(lam2, x, mu, var):

        C = (x - mu)

        return var/np.exp(-1) - np.dot( C**2 , np.exp(-lam2 * C**2 ) )

    def dxfx_var(lam2, x, mu):
        
        C = (x - mu)

        return np.dot( C**4, np.exp(-lam2 * C**2 ) ) + np.exp(-lam2 * C**2 ).sum()

    while abs( fx_var(lam, x, mu, var) ) > tol: #run the helper function and check
        lam = old_lam - fx_var(lam, x, mu, var)/dxfx_var(lam, x, mu)  # Newton-Raphson equation
        #print("Iteration" + str(i) + ": x = " + str(lam) + ", f(x) = " +  str( fx_var(lam, x, mu, var) ) )  
...
class MeanMaxEnt_Model(nn.Module):
    ...
    def fx_mean(self, lam1, x, mean):
        return mean/np.exp(-1) - np.sum( x * np.exp(-lam1 * x))/np.exp( -1 -lam1 * x).sum()
...
class VarianceMaxEnt_Model(nn.Module):
    ...
    def fx_var(self, lam2, x, mu, var):
        C = (x - mu)
        return var/np.exp(-1) - np.sum( np.exp(-lam2 * C**2 ) * C**2 )/np.sum( np.exp(-1 -lam2 * C**2 ) ).sum()
...
class MultipleMaxEnt_Model(nn.Module):
    ...
    def forward(self, x, mu, var, lam1=0, lam2=0, max_iter = 1000, tol = 1e-15):
        ...
        while abs( self.compute_fx(lam1, lam2, x, mu, var).mean() ) > tol: #run the helper function and check
          ...
          old_lam = lam
          i += 1
          #check if lam is updated or not
          if i > max_iter:
            break
